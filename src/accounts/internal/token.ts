const TOKEN_PREFIX = "asl_session_";
const TOKEN_BYTES = 32;
const SESSION_LIFETIME_MS = 7 * 24 * 60 * 60 * 1000;

export type NewAccountSession = {
  token: string;
  tokenHash: string;
  createdAt: number;
  expiresAt: number;
...
export async function createAccountSession(): Promise<NewAccountSession> {
...
export function isSessionToken(value: string): boolean {
  return /^asl_session_[A-Za-z0-9_-]{43}$/.test(value);
}
